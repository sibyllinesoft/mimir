...
class IndexConfig(BaseModel):
    ...
    languages: list[str] = Field(
        default_factory=lambda: ["ts", "tsx", "js", "jsx", "md", "mdx", "json", "yaml"]
    )
    ...
    max_files_to_embed: int | None = None
    imports_policy: dict[str, bool] = Field(
        default_factory=lambda: {"types_for_first_order": True, "code_for_direct_imports": True}
    )
...
class IndexManifest(BaseModel):
    ...
    @classmethod
    def load_from_file(cls, file_path: str | Path) -> IndexManifest:
...
class SymbolEntry(BaseModel):
    ...
    type: SymbolType
    symbol: str | None = None  # Fully qualified symbol name
    path: str  # Source file path (relative to repo root)
    span: tuple[int, int]  # Byte offset range [start, end)
    sig: str | None = None  # Type signature (for definitions)
    caller: str | None = None  # Calling symbol (for calls)
    callee: str | None = None  # Called symbol (for calls)
    from_module: str | None = None  # Import source (for imports)
...
class SerenaGraph(BaseModel):
    ...
    @classmethod
    def load_from_jsonl(cls, file_path: str | Path) -> SerenaGraph:
        ...
        return cls(
            entries=entries,
            file_count=len({entry.path for entry in entries}),
            symbol_count=len({entry.symbol for entry in entries if entry.symbol}),
        )
...
class SnippetCollection(BaseModel):
    ...
    @classmethod
    def load_from_jsonl(cls, file_path: str | Path) -> SnippetCollection:
...
class SearchResult(BaseModel):
    ...
    def combined_score(self) -> float:
        ...
        return 1.0 * self.scores.vector + 0.9 * self.scores.symbol + 0.3 * self.scores.graph
