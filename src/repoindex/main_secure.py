...
from .security.audit import SecurityEvent, SecurityEventType, get_security_auditor
from .security.config import SecurityConfig, get_security_config, load_security_config_file
...
Security Environment Variables:
  ...
  MIMIR_AUDIT_LOG_FILE=/path/log   # Audit log location
        ...
        parser.add_argument("--config", type=Path, help="Security configuration file")
        ...
        mcp_parser.add_argument(
            "--generate-config", action="store_true", help="Generate default configuration and exit"
        )

        # Index command
        index_parser = subparsers.add_parser("index", help="Index a repository")
        index_parser.add_argument("repo_path", help="Path to repository to index")
        index_parser.add_argument("--language", default="ts", help="Primary language (default: ts)")
        index_parser.add_argument("--rev", help="Git revision to index (default: HEAD)")
        index_parser.add_argument("--output", type=Path, help="Output directory for index")

        # Search command
        search_parser = subparsers.add_parser("search", help="Search an index")
        search_parser.add_argument("index_id", help="Index identifier to search")
        search_parser.add_argument("query", help="Search query")
        search_parser.add_argument("--k", type=int, default=20, help="Number of results to return")

        # Configuration command
        config_parser = subparsers.add_parser("config", help="Manage security configuration")
        config_parser.add_argument(
            "--generate", action="store_true", help="Generate default configuration"
        )
        config_parser.add_argument("--output", type=Path, help="Output file for configuration")
        config_parser.add_argument("--validate", type=Path, help="Validate configuration file")

        # Audit command
        audit_parser = subparsers.add_parser("audit", help="Security audit operations")
        audit_parser.add_argument(
            "--report", action="store_true", help="Generate security audit report"
        )
        audit_parser.add_argument("--output", type=Path, help="Output file for report")
    ...
    async def run_mcp_server(self, args) -> None:
        ...
        try:
            ...
            server = MimirSecureServer(
                storage_dir=args.storage_dir, security_config=self.security_config
            )
        ...
        try:
            ...
            self.security_auditor.record_event(
                SecurityEvent(
                    ...
                    metadata={"repo_path": args.repo_path, "error": str(e)},
                )
            )
            raise
        ...
        try:
            ...
            self.security_auditor.record_event(
                SecurityEvent(
                    ...
                    metadata={"command": args.command, "security_enabled": not args.no_security},
                )
            )
