...
from ..security.server_middleware import SecurityMiddleware, create_security_middleware
...
class SecureMCPServer(MCPServer):
    ...
    def _get_or_generate_master_key(self) -> Optional[bytes]:
        ...
        print(f"Generated Master Key (set as environment variable):")
        print(f"export {self.security_config.master_key_env_var}={master_key_b64}")
        print(f"WARNING: Save this key securely - lost keys = lost data!")
        print(f"===============================\n")
...
class MimirSecureServer:
    ...
    def __init__(
        ...
        self.security_config = security_config or get_security_config()
        self.mcp_server: Optional[SecureMCPServer] = None
...
async def main() -> None:
    ...
    parser.add_argument(
        "--storage-dir",
        type=Path,
        help="Storage directory for indexes"
    )
    parser.add_argument(
        "--config-file",
        type=Path,
