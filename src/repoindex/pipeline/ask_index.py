...
class SymbolGraphNavigator:
    ...
    async def ask(
        ...
        evidence_symbols = await self._walk_symbol_graph(
            seed_symbols, serena_graph, intents
        )
        
        # Gather evidence snippets
        evidence_snippets = await self._gather_evidence(
            evidence_symbols, context_lines
        )
        
        # Synthesize answer
        answer = await self._synthesize_answer(
            question, intents, evidence_snippets
        )
    ...
    async def _parse_intent(self, question: str) -> List[Intent]:
        ...
        if not intents:
            ...
            common_words = {
                'what', 'is', 'how', 'does', 'the', 'a', 'an', 'this', 'that',
                'where', 'when', 'why', 'who', 'which', 'can', 'could', 'would',
                'should', 'will', 'do', 'did', 'have', 'has', 'had', 'be', 'am',
                'are', 'was', 'were', 'been', 'being', 'to', 'of', 'in', 'for',
                'on', 'with', 'by', 'from', 'at', 'about', 'into', 'through',
    ...
    async def _plan_symbols(
        ...
        for intent in intents:
            for target in intent.targets:
                ...
                # Partial match
                matches = [
                    entry for symbol, entry in symbol_lookup.items()
                    ...
                    if intent.intent_type == "usage":
                        ...
                        if symbol_entry.symbol in called_by:
                            for caller in called_by[symbol_entry.symbol]:
                                caller_entries = [
                                    e for e in serena_graph.entries
    ...
    async def _gather_evidence(
        self,
        evidence_symbols: List[SymbolEntry],
        context_lines: int
    ) -> List[CodeSnippet]:
    ...
    async def _synthesize_answer(
        ...
        if primary_intent:
            if primary_intent.intent_type == "definition":
                answer_parts.append(f"Based on the code analysis, here's what I found about {', '.join(primary_intent.targets)}:")
                ...
                answer_parts.append(f"Here are the usage patterns I found for {', '.join(primary_intent.targets)}:")
                ...
                answer_parts.append(f"Here are the dependencies I found for {', '.join(primary_intent.targets)}:")
                ...
                answer_parts.append(f"Here's the execution flow for {', '.join(primary_intent.targets)}:")
                ...
                answer_parts.append(f"Based on the code analysis:")
