...
from ..data.schemas import IndexManifest, IndexCounts, ArtifactPaths, ToolVersions
from ..util.fs import get_directory_size, atomic_write_bytes
...
class BundleCreator:
    ...
    async def _create_compressed_bundle(
        ...
        if total_size > self.max_bundle_size:
            print(f"Warning: Bundle size ({total_size / 1024 / 1024:.1f}MB) exceeds recommended limit")
