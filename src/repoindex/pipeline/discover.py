...
from ..data.schemas import RepoInfo, IndexConfig
from ..util.gitio import GitRepository, discover_git_repository
...
class FileDiscovery:
    ...
    async def discover_files(
        ...
        if excludes is None:
            excludes = [
                ...
                "__pycache__/", ".git/", ".vscode/", ".idea/", "target/",
                ".cache/", "tmp/", "temp/"
            ]
        
        # Get tracked files from git
        tracked_files = await asyncio.to_thread(
            self.git_repo.list_tracked_files,
        ...
        for file_path in current_files:
            try:
                ...
                current_hash = await asyncio.to_thread(
                    self.git_repo.hash_file_content, file_path
                )
    ...
    async def get_file_metadata(self, file_paths: List[str]) -> Dict[str, Dict]:
        ...
        for i in range(0, len(file_paths), batch_size):
            ...
            batch_metadata = await asyncio.gather(
                *[self._get_single_file_metadata(fp) for fp in batch],
                return_exceptions=True
            )
            
            for file_path, meta in zip(batch, batch_metadata):
    ...
    async def get_cache_key(self, config: IndexConfig) -> str:
        ...
        return await asyncio.to_thread(
            self.git_repo.get_cache_key, config_hash
        )
    ...
    async def get_priority_files(
        ...
        def file_importance(file_path: str) -> int:
            ...
            if filename in {'package.json', 'tsconfig.json', 'index.ts', 'index.js', 'main.ts', 'main.js'}:
                score += 15
