...
class HybridSearchEngine:
    ...
    async def search(
        ...
        serena_graph: Optional[SerenaGraph],
        repomap: Optional[RepoMap],
        repo_root: str,
        rev: str,
        ...
        async with self.trace_manager.trace_search_request(
            ...
            if features.vector and vector_index:
                search_tasks.append(
                    self._vector_search(query, vector_index)
                )
            
            if features.symbol and serena_graph:
                search_tasks.append(
                    self._symbol_search(query, serena_graph)
                )
    ...
    async def _graph_expansion(
        ...
        candidate_files = set(path for path, _, _ in candidates)
