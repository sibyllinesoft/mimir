...
from ..data.schemas import IndexConfig, VectorChunk, VectorIndex
from ..util.fs import atomic_write_bytes, atomic_write_json
...
class LEANNAdapter:
    ...
    async def build_index(
        ...
        processable_files = await self._filter_processable_files(repo_root, files, repomap_order)

        if not processable_files:
            # Return empty index if no processable files
            return VectorIndex(
                chunks=[], dimension=self.dimension, total_tokens=0, model_name=self.model_name
            )
    ...
    async def _filter_processable_files(
        self, repo_root: Path, files: list[str], repomap_order: list[str]
    ) -> list[str]:
    ...
    async def _chunk_by_functions(self, content: str, file_path: str) -> list[VectorChunk]:
        """
        ...
        for i, line in enumerate(lines):
            ...
            if (
                ...
                and any(
                    keyword in stripped
                    for keyword in ["function ", "def ", "class ", "interface ", "async "]
                )
                and "{" in stripped
                or ":" in stripped
            ):
    ...
    async def _create_chunk_from_lines(
        self, lines: list[str], file_path: str, start_line: int
    ) -> VectorChunk | None:
    ...
    async def _chunk_by_tokens(self, content: str, file_path: str) -> list[VectorChunk]:
    ...
    async def _generate_embeddings(
        self, chunks: list[VectorChunk], progress_callback: Callable[[int], None] | None = None
    ) -> None:
    ...
    async def _save_index_artifacts(self, vector_index: VectorIndex, work_dir: Path) -> None:
        ...
        if vector_index.chunks:
            embeddings = np.array(
                [chunk.embedding for chunk in vector_index.chunks if chunk.embedding]
            )
    ...
    async def search_similar(
        self, query_text: str, vector_index: VectorIndex, k: int = 20
    ...
    async def validate_index(self, vector_index: VectorIndex) -> dict[str, Any]:
        """
        ...
        validation_report["statistics"]["avg_tokens_per_chunk"] = total_tokens / len(
            vector_index.chunks
        )

        chunks_with_embeddings = sum(1 for chunk in vector_index.chunks if chunk.embedding)
        validation_report["statistics"]["embedding_coverage"] = chunks_with_embeddings / len(
            vector_index.chunks
        )
