...
from ..data.schemas import RepoMap, FileRank, DependencyEdge
...
class RepoMapperAdapter:
    ...
    def __init__(self, repomapper_path: Optional[str] = None, validate: bool = True):
    ...
    async def analyze_file_subset(
        ...
        cmd = [
            self.repomapper_path,
            "--repo-root", str(repo_root),
            "--files-list", str(all_files_path),
            "--focus-files", str(focus_files_path),
            "--output", str(output_path),
            "--format", "json",
            "--include-edges"
        ]
