...
from ..security.sandbox import ResourceLimiter, configure_sandbox
...
class SecureIndexingPipeline:
    ...
    async def _execute_secure_pipeline(self, context: SecurePipelineContext) -> None:
        ...
        self.metrics_collector.record_pipeline_start(
            "secure_pipeline", str(context.config.languages)
        )
        ...
        with enhanced_logger.performance_track("secure_pipeline_execution"):
            async with self.trace_manager.trace_operation(
                ...
                try:
                    ...
                    context.security_auditor.record_event(
                        SecurityEvent(
                    ...
                    await self._execute_secure_stage(
                        context, "acquire", self._stage_secure_acquire, error_collector
                    )
                    if context.cancelled:
                        return

                    await self._execute_secure_stage(
                        context, "repomapper", self._stage_secure_repomapper, error_collector
                    )
                    if context.cancelled:
                        return

                    await self._execute_secure_stage(
                        context, "serena", self._stage_secure_serena, error_collector
                    )
                    if context.cancelled:
                        return

                    await self._execute_secure_stage(
                        context, "leann", self._stage_secure_leann, error_collector
                    )
                    if context.cancelled:
                        return

                    await self._execute_secure_stage(
                        context, "snippets", self._stage_secure_snippets, error_collector
                    )
                    if context.cancelled:
                        return

                    await self._execute_secure_stage(
                        context, "bundle", self._stage_secure_bundle, error_collector
                    )
                    if context.cancelled:
                        return
                    ...
                    self.metrics_collector.record_pipeline_error(
                        "secure_pipeline", error_type, severity, str(context.config.languages)
                    )
                    ...
                    try:
                        ...
                        enhanced_logger.operation_error(
                            "secure_pipeline_cleanup", cleanup_error, pipeline_id=context.index_id
                        )
            ...
            context.security_auditor.record_event(
                SecurityEvent(
                    ...
                    metadata={
                        ...
                        "error": str(e),
                        "recovery_strategy": (
                            e.recovery_strategy.value if e.recovery_strategy else "unknown"
                        ),
                    },
                )
            )
    ...
    async def _stage_secure_acquire(self, context: SecurePipelineContext) -> None:
        ...
        async with self.trace_manager.trace_pipeline_stage(
            ...
            with enhanced_logger.performance_track(
                "secure_file_discovery", include_system_metrics=True
            ):
                ...
                try:
                    async with self.io_semaphore:
                        ...
                        secure_discovery = SecureFileDiscovery(
                            original_discovery, context.security_config
                        )
    ...
    async def _stage_secure_repomapper(self, context: SecurePipelineContext) -> None:
        ...
        async with self.trace_manager.trace_pipeline_stage(
            "secure_repomapper", context.index_id, files_count=len(context.tracked_files)
        ) as span:
            with enhanced_logger.performance_track(
                "secure_repository_analysis", include_system_metrics=True
            ):
    ...
    def _update_stage_progress(
        self, context: SecurePipelineContext, stage: PipelineStage, stage_progress: int
    ) -> None:
