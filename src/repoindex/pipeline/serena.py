...
class SerenaAdapter:
    ...
    def __init__(self, serena_path: Optional[str] = None, validate: bool = True):
    ...
    async def analyze_project(
        ...
        serena_config = await self._create_serena_config(
            project_root, ts_files, work_dir, config
        )
        ...
        if config.imports_policy.get("types_for_first_order", True):
            await self._extract_dependency_types(
                project_root, serena_config, work_dir
            )
        ...
        if config.imports_policy.get("code_for_direct_imports", True):
            await self._extract_vendor_sources(
                project_root, ts_files, work_dir
            )
        ...
        graph = SerenaGraph(
            ...
            symbol_count=len(set(entry.symbol for entry in symbol_entries if entry.symbol))
        )
    ...
    def _parse_symbol_entry(self, data: Dict[str, Any]) -> Optional[SymbolEntry]:
    ...
    async def _extract_dependency_types(
        ...
        try:
            ...
            if result.returncode != 0:
                ...
                print(f"Warning: Type extraction failed: {stderr.decode() if stderr else 'Unknown error'}")
    ...
    async def _extract_vendor_sources(
        ...
        cmd = [
            ...
            "extract-vendor",
            "--project", str(project_root),
            "--files", str(files_list_path),
            "--output", str(vendor_dir),
            "--direct-imports-only"
        ]
        ...
        try:
            ...
            if result.returncode != 0:
                ...
                print(f"Warning: Vendor source extraction failed: {stderr.decode() if stderr else 'Unknown error'}")
        ...
        try:
            ...
            if validation_report["typescript_files"] == 0:
                validation_report["issues"].append(
                    "No TypeScript files found for analysis"
                )
