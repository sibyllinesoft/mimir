...
from ..data.schemas import CodeSnippet, SerenaGraph, SnippetCollection, SymbolEntry
...
class SnippetExtractor:
    ...
    async def extract_snippets(
        ...
        snippet_collection = SnippetCollection(snippets=all_snippets, total_count=len(all_snippets))
    ...
    async def _extract_file_snippets(
        self, repo_root: Path, file_path: str, entries: list[SymbolEntry], context_lines: int
    ) -> list[CodeSnippet]:
    ...
    async def _extract_single_snippet(
        self, content: str, file_path: str, start_byte: int, end_byte: int, context_lines: int
    ) -> CodeSnippet | None:
    ...
    async def verify_snippet_integrity(self, repo_root: Path, snippet: CodeSnippet) -> bool:
        """
    ...
    async def update_snippet_content(
        self, repo_root: Path, snippet: CodeSnippet, context_lines: int = 5
    ) -> CodeSnippet | None:
        """
    ...
    async def merge_overlapping_snippets(
        self, snippets: list[CodeSnippet], max_gap: int = 100
    ) -> list[CodeSnippet]:
        """
