...
class SecurityConfig:
    ...
    def from_environment(cls) -> "SecurityConfig":
        ...
        # Authentication settings
        config.require_authentication = _get_env_bool(
            "MIMIR_REQUIRE_AUTH", config.require_authentication
        )
        config.auth_token_lifetime = _get_env_int(
            "MIMIR_AUTH_TOKEN_LIFETIME", config.auth_token_lifetime
        )
        ...
        config.api_key_rate_limit = _get_env_int(
            "MIMIR_API_KEY_RATE_LIMIT", config.api_key_rate_limit
        )
        ...
        # Credential scanning
        config.enable_credential_scanning = _get_env_bool(
            "MIMIR_ENABLE_CREDENTIAL_SCANNING", config.enable_credential_scanning
        )
        config.max_scan_files = _get_env_int("MIMIR_MAX_SCAN_FILES", config.max_scan_files)

        # Sandbox settings
        config.enable_sandboxing = _get_env_bool(
            "MIMIR_ENABLE_SANDBOXING", config.enable_sandboxing
        )
        config.max_memory_mb = _get_env_int("MIMIR_MAX_MEMORY_MB", config.max_memory_mb)
        config.max_cpu_time_seconds = _get_env_int(
            "MIMIR_MAX_CPU_TIME", config.max_cpu_time_seconds
        )
        config.max_wall_time_seconds = _get_env_int(
            "MIMIR_MAX_WALL_TIME", config.max_wall_time_seconds
        )

        # Encryption settings
        config.enable_index_encryption = _get_env_bool(
            "MIMIR_ENABLE_ENCRYPTION", config.enable_index_encryption
        )
        config.encrypt_embeddings = _get_env_bool(
            "MIMIR_ENCRYPT_EMBEDDINGS", config.encrypt_embeddings
        )
        config.encrypt_metadata = _get_env_bool("MIMIR_ENCRYPT_METADATA", config.encrypt_metadata)

        # Audit logging
        config.enable_audit_logging = _get_env_bool(
            "MIMIR_ENABLE_AUDIT_LOGGING", config.enable_audit_logging
        )
        ...
        config.audit_log_max_size_mb = _get_env_int(
            "MIMIR_AUDIT_LOG_MAX_SIZE_MB", config.audit_log_max_size_mb
        )
        config.audit_log_backup_count = _get_env_int(
            "MIMIR_AUDIT_LOG_BACKUP_COUNT", config.audit_log_backup_count
        )

        # Security monitoring
        config.enable_threat_detection = _get_env_bool(
            "MIMIR_ENABLE_THREAT_DETECTION", config.enable_threat_detection
        )
        config.max_auth_failures = _get_env_int("MIMIR_MAX_AUTH_FAILURES", config.max_auth_failures)
        config.ip_block_duration_minutes = _get_env_int(
            "MIMIR_IP_BLOCK_DURATION", config.ip_block_duration_minutes
        )

        # Abuse prevention
        config.enable_abuse_prevention = _get_env_bool(
            "MIMIR_ENABLE_ABUSE_PREVENTION", config.enable_abuse_prevention
        )
        config.suspicious_pattern_threshold = _get_env_int(
            "MIMIR_SUSPICIOUS_THRESHOLD", config.suspicious_pattern_threshold
        )
        config.error_rate_threshold = _get_env_int(
            "MIMIR_ERROR_RATE_THRESHOLD", config.error_rate_threshold
        )
    ...
    def from_file(cls, config_file: Path) -> "SecurityConfig":
        ...
        try:
            ...
            logger.error(
                "Failed to load security config from file",
    ...
    def to_file(self, config_file: Path) -> None:
        ...
        try:
            ...
            logger.error(
                "Failed to save security config to file", config_file=str(config_file), error=str(e)
            )
            raise
