...
class CryptoManager:
    ...
    def verify_hmac(
        self, data: bytes, expected_hmac: str, key: bytes, algorithm: str = "sha256"
    ) -> bool:
        """Verify HMAC of data.
...
class FileEncryption:
    ...
    def decrypt_file(
        self, input_path: Path, output_path: Path, password: str | None = None
    ) -> dict[str, Any]:
        """Decrypt a file.
    ...
    def encrypt_data(
        self, data: bytes, password: str | None = None
    ...
    def decrypt_data(
        self, encrypted_data: bytes, metadata: dict[str, Any], password: str | None = None
    ) -> bytes:
...
class IndexEncryption:
    ...
    def encrypt_vector_index(
        ...
        """
        results = {"encrypted_files": [], "skipped_files": [], "errors": []}
        ...
        try:
            for file_path in index_dir.rglob("*"):
                ...
                try:
                    ...
                    logger.error(
                        "Index file encryption failed", file_path=str(file_path), error=str(e)
                    )
    ...
    def decrypt_vector_index(self, index_dir: Path, password: str | None = None) -> dict[str, Any]:
        ...
        """
        results = {"decrypted_files": [], "errors": []}
            ...
            for encrypted_file in index_dir.rglob("*.enc"):
                try:
                    ...
                    decryption_result = self.file_encryption.decrypt_file(
                        input_path=encrypted_file, output_path=original_path, password=password
                    )
                    ...
                    results["errors"].append(error_msg)
                    logger.error(
                        "Index file decryption failed", file_path=str(encrypted_file), error=str(e)
                    )
...
_file_encryption: FileEncryption | None = None
_index_encryption: IndexEncryption | None = None
...
def configure_crypto(master_key: bytes | None = None) -> None:
