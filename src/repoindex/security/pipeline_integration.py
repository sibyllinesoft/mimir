...
from ..data.schemas import IndexConfig, PipelineStage
from ..util.log import get_logger
from ..util.errors import MimirError, ErrorSeverity, ErrorCategory, RecoveryStrategy

from .validation import (
    PathValidator, ContentValidator, SchemaValidator,
    SecurityValidationError, ValidationContext
)
from .sandbox import ResourceLimiter, ProcessIsolator, SandboxExecutionError
from .auth import SecurityAuditor, SecurityEvent, SecurityEventType
from .secrets import CredentialScanner, CredentialPattern
...
class SecureFileDiscovery:
    ...
    def __init__(self, file_discovery, security_config: Optional[SecurityConfig] = None):
    ...
    async def discover_files(
        ...
        try:
            ...
            if not self.path_validator.validate_path(
                str(self.file_discovery.repo_root), 
                operation="read"
            ):
            ...
            for file_path in discovered_files:
                try:
                    ...
                    if full_path.exists():
                        ...
                        if (self.security_config.enable_credential_scanning and 
                            self._should_scan_file(file_path)):
                            ...
                            try:
                                ...
                                if credentials:
                                    ...
                                    self.security_auditor.record_event(SecurityEvent(
                                        ...
                                        metadata={
                                            "file_path": file_path,
                                            "credential_types": [c.pattern_type for c in credentials],
            ...
            if security_violations:
                self.security_auditor.record_event(SecurityEvent(
                    ...
                    component="file_discovery",
                    severity="medium",
                    message=f"Security violations detected during file discovery",
                    metadata={
...
class SecurePipelineContext:
    ...
    def __init__(self, original_context, security_config: Optional[SecurityConfig] = None):
    ...
    async def secure_external_tool_execution(
        ...
        if not self.security_config.enable_sandboxing:
            ...
            self.security_auditor.record_event(SecurityEvent(
                ...
                severity="medium",
                message=f"External tool executed without sandboxing",
                metadata={
                    "tool": tool_name,
        ...
        try:
            ...
            self.security_auditor.record_event(SecurityEvent(
                ...
                metadata={
                    "tool": tool_name,
                    "violation_type": e.violation_type if hasattr(e, 'violation_type') else "unknown",
                    "error": str(e)
                }
            ))
            raise
            ...
            for key, value in validated_inputs.items():
                if isinstance(value, str):
                    ...
                    if any(char in value for char in ['<', '>', '&', '"', '\x00']):
                        sanitized = ''.join(char for char in value if char not in ['<', '>', '&', '"', '\x00'])
...
class SecurityPipelineWrapper:
    ...
    def __init__(self, original_pipeline, security_config: Optional[SecurityConfig] = None):
...
def create_secure_pipeline(original_pipeline, security_config: Optional[SecurityConfig] = None):
...
def create_secure_file_discovery(original_discovery, security_config: Optional[SecurityConfig] = None):
