...
class CredentialScanner:
    ...
    def scan_file(self, file_path: Path) -> list[dict[str, Any]]:
        ...
        try:
            ...
            logger.error(
                "Failed to scan file for credentials", file_path=str(file_path), error=str(e)
            )
            return []
        ...
        try:
            for file_path in directory.rglob("*"):
                if files_scanned >= max_files:
                    logger.warning(
                        "Maximum file limit reached for credential scanning", max_files=max_files
                    )
                    break
            ...
            logger.error(
                "Failed to scan directory for credentials", directory=str(directory), error=str(e)
            )
