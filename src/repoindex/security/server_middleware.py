...
from .auth import AuthManager, AuthenticationFailed, AuthorizationDenied, RateLimitExceeded
from .sandbox import get_sandbox, SandboxViolation
from .secrets import CredentialScanner
from .audit import get_security_auditor, SecurityEvent, SecurityEventType, SecurityEventSeverity
...
class SecurityMiddleware:
    ...
    def secure_tool(
        ...
        scan_credentials: Optional[bool] = None,
            ...
            async def wrapper(*args, **kwargs) -> CallToolResult:
                ...
                try:
                    ...
                    return await self._handle_security_error(
                        e, tool_name, arguments, client_ip, 
    ...
    async def _check_rate_limits(
        ...
        try:
            ...
            rate_limit_info = self.auth_manager.rate_limiter.check_rate_limit(
                client_ip=client_ip,
                api_key=auth_result.get("api_key_obj"),
                endpoint=tool_name
            )
    ...
    async def _scan_repository_credentials(
        ...
        try:
            ...
            logger.error(
                "Credential scanning failed",
                repo_path=repo_path,
                error=str(e),
                user_id=user_id
            )
    
    def _should_scan_credentials(self, scan_override: Optional[bool] = None) -> bool:
...
def create_security_middleware(
    ...
    if require_auth and api_keys_file and not auth_manager.api_key_validator.api_keys:
        ...
        print(f"Default API Key Generated:")
        print(f"Key ID: {key_id}")
        print(f"API Key: {raw_key}")
        print(f"Save this key securely - it will not be shown again!")
        print(f"===============================\n")
