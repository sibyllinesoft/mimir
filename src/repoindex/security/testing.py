...
from .validation import InputValidator, PathValidator, ContentValidator, SecurityViolation
from .auth import AuthManager, APIKeyValidator, RateLimiter, AuthenticationFailed, RateLimitExceeded
from .sandbox import Sandbox, SandboxViolation, ResourceLimiter
from .secrets import CredentialScanner, SecretsManager
from .audit import SecurityAuditor, SecurityEvent, SecurityEventType, SecurityEventSeverity
...
class SecurityTestFramework:
    ...
    def run_all_tests(self) -> Dict[str, Any]:
        ...
        for category_name, test_func in test_categories:
            try:
                ...
                results["test_summary"]["critical_failures"] += category_results.get("critical_failures", 0)
        ...
        results["test_summary"]["duration"] = results["test_summary"]["end_time"] - results["test_summary"]["start_time"]
        results["test_summary"]["success_rate"] = (
            results["test_summary"]["tests_passed"] / results["test_summary"]["tests_run"]
            if results["test_summary"]["tests_run"] > 0 else 0
        )
    ...
    async def _test_sandboxing(self) -> Dict[str, Any]:
        ...
        results["tests_run"] += 1
        try:
            result = await sandbox.run_sandboxed_command(
        ...
        results["tests_run"] += 1
        try:
            resource_limiter = ResourceLimiter({"max_wall_time": 1})
    ...
    def _test_encryption(self) -> Dict[str, Any]:
        ...
        try:
            ...
            decryption_result = file_encryption.decrypt_file(
