...
class PathValidator:
    ...
    def validate_path(self, path: str | Path, operation: str = "access") -> Path:
        ...
        """
        if not path:
            raise SecurityViolation("Empty path provided", "empty_path", {"operation": operation})
    ...
    def validate_repository_path(self, repo_path: str | Path) -> Path:
        ...
        if not git_dir.exists():
            logger.warning(
                "Repository path does not contain .git directory", repo_path=str(validated_path)
            )

        return validated_path

    def validate_file_extension(self, file_path: str | Path) -> bool:
...
class ContentValidator:
    ...
    def validate_file_content(
        self, file_path: Path, content: bytes | None = None
    ) -> dict[str, Any]:
        ...
        if content is None:
            if not file_path.exists():
                raise SecurityViolation(
                    "File does not exist", "file_not_exists", {"path": str(file_path)}
                )
        ...
        if null_bytes > 0:
            suspicious_matches.append(
                {"pattern": "null_bytes", "matches": null_bytes, "examples": ["\\x00"]}
            )
...
class SchemaValidator:
    ...
    def validate_data(
        self, data: Any, schema: dict[str, Any], schema_id: str = "default"
    ) -> dict[str, Any]:
        ...
        if errors:
            ...
            raise SecurityViolation(
                ...
                {"error_count": len(errors), "errors": error_details, "schema_id": schema_id},
            )

        return {"valid": True, "schema_id": schema_id, "data_type": type(data).__name__}
...
class InputValidator:
    ...
    def validate_mcp_request(self, tool_name: str, arguments: dict[str, Any]) -> dict[str, Any]:
        ...
        """
        results = {"tool_name": tool_name, "validation_passed": True, "warnings": []}
        ...
        logger.info(
            "MCP request validation passed", tool_name=tool_name, warnings=results["warnings"]
        )
        return results
