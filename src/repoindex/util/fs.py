...
def atomic_write_bytes(file_path: str | Path, content: bytes) -> None:
...
def read_text_with_hash(file_path: str | Path) -> tuple[str, str]:
...
def read_bytes_with_hash(file_path: str | Path) -> tuple[bytes, str]:
...
def create_temp_directory(prefix: str = "mimir_", parent: Path | None = None) -> Path:
...
class TemporaryDirectory:
    ...
    def __init__(self, prefix: str = "mimir_", parent: Path | None = None):
        self.prefix = prefix
        self.parent = parent
...
def extract_file_span(
    file_path: str | Path, start_byte: int, end_byte: int, context_lines: int = 5
) -> tuple[str, str, str]:
    """
