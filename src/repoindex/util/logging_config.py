...
PIPELINE_ID: ContextVar[Optional[str]] = ContextVar('pipeline_id', default=None)
...
def log_external_tool_call(
    ...
    logger.log(
        ...
        extra={
            ...
            'extra_fields': {
                'tool': tool,
                'command': command[:3] + ['...'] if len(command) > 3 else command,  # Truncate long commands
                'success': exit_code == 0
            }
        }
    )