...
'''
    ...
    async def test_full_pipeline_execution(self, test_repo: Path, pipeline_runner: PipelineRunner):
        ...
        try:
            ...
            if "repomapper" in str(e).lower() or "serena" in str(e).lower() or "leann" in str(e).lower():
    ...
    async def test_search_functionality(self, test_repo: Path, mcp_server: MimirMCPServer):
        ...
        if index_result.get("error"):
            if any(tool in index_result["error"].lower() for tool in ["repomapper", "serena", "leann"]):
...
'''
        ...
        subprocess.run(["git", "commit", "-m", "Add auth middleware"], cwd=test_repo, capture_output=True)
        ...
        middleware_found = any("requireAuth" in r.get("content", "") for r in search_result["results"])
        assert middleware_found, "Should find new requireAuth middleware"
        
        print(f"Incremental indexing successful:")
