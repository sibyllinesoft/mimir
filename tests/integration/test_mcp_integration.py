...
from src.repoindex.data.schemas import IndexState
...
'''
            ...
            def subprocess_side_effect(*args, **kwargs):
                ...
                if 'repomapper' in cmd[0]:
                    if '--version' in cmd:
                        ...
                        return Mock(returncode=0, stdout=json.dumps(mock_responses["repomapper"]), stderr="")
                elif 'serena' in cmd[0]:
                    if '--version' in cmd:
                        return Mock(returncode=0, stdout="serena 1.0.0", stderr="")
                    else:
                        return Mock(returncode=0, stdout=json.dumps(mock_responses["serena"]), stderr="")
                elif 'leann' in cmd[0]:
                    if '--version' in cmd:
                        return Mock(returncode=0, stdout="leann 1.0.0", stderr="")
                    else:
                        return Mock(returncode=0, stdout=json.dumps(mock_responses["leann"]), stderr="")
                else:
    ...
    async def test_mcp_resources(self, test_repo: Path, mcp_server: MCPServer):
        ...
        status_result = await mcp_server._get_status_resource(index_id, mcp_server.indexes_dir / index_id)
        ...
        print(f"✅ Status resource read successfully")
