...
from src.repoindex.data.schemas import IndexConfig, IndexManifest, PipelineStatus, RepoInfo
...
"""
    ...
    async def test_complete_pipeline_flow(self, pipeline_runner, integration_repo, storage_dir):
        ...
        repo_info = RepoInfo(root=str(integration_repo), rev="main", worktree_dirty=False)
        ...
        with patch("src.repoindex.pipeline.repomapper.RepoMapperAdapter") as mock_repomapper:
            with patch("src.repoindex.pipeline.serena.SerenaAdapter") as mock_serena:
                with patch("src.repoindex.pipeline.leann.LEANNAdapter") as mock_leann:
                    ...
                    mock_repomapper_instance.analyze.return_value = RepoMap(file_ranks=[], edges=[])

                    mock_serena_instance.analyze.return_value = SerenaGraph(entries=[])

                    mock_leann_instance.embed_files.return_value = VectorIndex(chunks=[])
            ...
            with patch("src.repoindex.pipeline.serena.SerenaAdapter") as mock_serena:
                with patch("src.repoindex.pipeline.leann.LEANNAdapter") as mock_leann:
                    ...
                    await pipeline_runner.run_pipeline(
                        manifest, status_callback=mock_status_callback
                    )
    ...
    async def test_pipeline_cancellation(self, pipeline_runner, integration_repo):
        ...
        with patch("src.repoindex.pipeline.serena.SerenaAdapter") as mock_serena:
            ...
            # Start pipeline
            pipeline_task = asyncio.create_task(pipeline_runner.run_pipeline(manifest))
    ...
    async def test_ask_integration(self, storage_dir, integration_repo):
        ...
        serena_graph = SerenaGraph(
            entries=[
                ...
                # Calculator usage
                SymbolEntry(
                    type=SymbolType.REF, path="src/index.ts", span=(3, 13), symbol="Calculator"
                ),
            ]
        )
            ...
            with patch("src.repoindex.pipeline.serena.SerenaAdapter"):
                with patch("src.repoindex.pipeline.leann.LEANNAdapter"):
                    ...
                    with patch.object(server, "_execute_search") as mock_search:
                        from src.repoindex.data.schemas import FeatureConfig, SearchResponse
