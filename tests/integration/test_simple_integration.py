...
'''
        ...
        print(f"Git validation: returncode={test_result.returncode}, stdout='{test_result.stdout.strip()}', stderr='{test_result.stderr.strip()}'")
        ...
        print(f"Git status: returncode={status_result.returncode}, output='{status_result.stdout.strip()}'")
        ...
        try:
            ...
            with patch('src.repoindex.pipeline.repomapper.RepoMapperAdapter') as mock_repomapper_cls, \
                 ...
                 patch('src.repoindex.pipeline.discover.discover_git_repository') as mock_git_discovery:
                
                # Configure RepoMapper mock
                from src.repoindex.data.schemas import RepoMap, FileRank, DependencyEdge
                ...
                assert index_result.isError is False, f"Indexing failed: {index_result.content[0].text}"
                ...
                bundle_result = await mcp_server._get_repo_bundle({
                    "index_id": index_id
                })
                ...
                assert len(symbol_graph["entries"]) >= 2  # Should have AuthController and login method
