...
class TestRepoMapperAdapter:
    ...
    async def test_analyze_repomapper_failure(self, adapter, temp_dir):
        ...
        with patch('asyncio.create_subprocess_exec') as mock_subprocess:
            mock_process = AsyncMock()
            mock_process.communicate.return_value = (b"", b"RepoMapper analysis failed: syntax error")
    ...
    async def test_analyze_invalid_json(self, adapter, temp_dir):
        ...
        with patch('asyncio.create_subprocess_exec') as mock_subprocess:
            ...
            assert ("json" in error_msg or "parse" in error_msg or 
                    "property name" in error_msg or "double quotes" in error_msg)
...
class TestSerenaAdapter:
    ...
    async def test_analyze_serena_failure(self, adapter, temp_dir):
        ...
        with patch('asyncio.create_subprocess_exec') as mock_subprocess:
            mock_process = AsyncMock()
            mock_process.stderr.readline = AsyncMock(side_effect=[b"Serena analysis failed: TypeScript parsing error", None])
            mock_process.returncode = 1
            mock_subprocess.return_value = mock_process
...
''')
        ...
        return [
            str(src_dir / "calculator.ts"),
            str(src_dir / "utils.ts")
        ]
    ...
    @pytest.mark.asyncio
    async def test_embed_files_success(self, adapter, sample_file_contents, sample_leann_embeddings, temp_dir):
...
''')
        ...
        processed_files = set(chunk.path for chunk in result.chunks)
