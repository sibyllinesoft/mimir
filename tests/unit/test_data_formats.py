...
class TestDataFormatCompatibility:
    ...
    def test_repomapper_real_world_format(self):
        ...
        repomapper_data = {
            ...
            "metadata": {
                ...
                "language_distribution": {"typescript": 0.8, "javascript": 0.15, "json": 0.05},
            },
        }
        ...
        repomap = RepoMap(file_ranks=file_ranks, edges=edges, total_files=len(file_ranks))
    ...
    def test_serena_real_world_format(self):
        ...
        unique_files = {entry.path for entry in entries}
        graph = SerenaGraph(
            entries=entries, file_count=len(unique_files), symbol_count=len(entries)
        )
    ...
    def test_leann_real_world_format(self):
        ...
        for orig, rest in zip(vector_index.chunks, restored.chunks, strict=False):
    ...
    def test_search_response_format(self):
        ...
        scores = SearchScores(vector=0.85, symbol=0.92, graph=0.67)
...
class TestIndexManifestFormat:
    ...
    def test_complete_manifest_structure(self, temp_dir):
        ...
        repo_info = RepoInfo(
            root=str(temp_dir), rev="feature/authentication-v2", worktree_dirty=True
        )
    ...
    def test_manifest_file_persistence(self, temp_dir):
        ...
        # Create manifest
        repo_info = RepoInfo(root=str(temp_dir / "test_repo"), rev="main", worktree_dirty=False)

        manifest = IndexManifest(repo=repo_info, config=IndexConfig())
